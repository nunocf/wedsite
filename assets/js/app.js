...
var groomMapDiv, brideMapDiv, churchMapDiv, restaurantMapDiv;

var groomMap, brideMap, churchMap, restaurantMap;
...
app.ports.initMaps.subscribe(locations => {
  requestAnimationFrame(() => {
    ...
    const getLatLng = callee => ({
      lat: callee.getCenter().lat,
      lng: callee.getCenter().lng()
    });

    groomMap.addListener("drag", () => {
      app.ports.mapMoved.send({
        location: getLatLng(groomMap),
        name: "groomMap"
      });
    });
    ...
    restaurantMap.addListener("drag", () => {
      app.ports.mapMoved.send({
        location: getLatLng(restaurantMap),
        name: "restaurantMap"
      });
    });
  });
